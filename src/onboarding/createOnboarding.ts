...
const PHOTOS: readonly PhotoDef[] = [
  { src: "3.jpg",  depth: 3,   dir:  1, left: "67.57%", top: "-11.3%",  width: "19.79%", height: "48.15%" },
  { src: "5.jpg",  depth: 2,   dir: -1, left: "-5.78%", top: "34.8%",   width: "23.26%", height: "25.11%" },
  { src: "13.jpg", depth: 3,   dir: -1, left: "4.1%",   top: "70.91%",  width: "19.65%", height: "47.82%" },
  { src: "7.jpg",  depth: 2.6, dir:  1, left: "61.8%",  top: "68.39%",  width: "18.01%", height: "43.89%" },
  { src: "12.jpg", depth: 2.4, dir:  1, left: "84.79%", top: "64.75%",  width: "16.25%", height: "39.54%" },
  { src: "17.jpg", depth: 1.2, dir:  1, left: "91.04%", top: "9.98%",   width: "16.6%",  height: "17.94%" },
  { src: "14.jpg", depth: 0.9, dir: -1, left: "17.29%", top: "6.53%",   width: "11.67%", height: "12.61%" },
  { src: "11.jpg", depth: 2.2, dir: -1, left: "23.26%", top: "-27.7%",  width: "16.67%", height: "40.55%" },
  { src: "16.jpg", depth: 1.4, dir:  1, left: "74.93%", top: "45.81%",  width: "10.21%", height: "24.94%" },
  { src: "10.jpg", depth: 0.7, dir:  1, left: "47.64%", top: "84.32%",  width: "9.72%",  height: "10.51%" },
  { src: "6.jpg",  depth: 0.5, dir: -1, left: "15.47%", top: "43.69%",  width: "7.81%",  height: "8.45%"  },
];

/** `null` marks the petal slot inside the "1927 · 2027." line. */
...
// The Google/Fontsource web build of Libertinus Serif ships blank glyphs for
// every precomposed Vietnamese letter carrying circumflex/breve/horn PLUS a
// second mark. Each renders correctly as base + combining mark instead.
...
const GLYPH_RE = new RegExp("[" + Object.keys(GLYPH_FIX).join("") + "]", "g");
...
export function createOnboarding({
    ...
    const words = line
      .map((word) =>
        word === null
          ...
          : `<span class="ob-word">${word}</span>`,
      )
      .join("");
    return `<div class="ob-headline__line${tight ? " ob-headline__line--tight" : ""}">${words}</div>`;
  }).join("");

  const promptHtml = PROMPT_LINES.map(
    (line) =>
      `<div class="ob-prompt__line">${line
        .map((word) => `<span class="ob-prompt__word">${word}</span>`)
        .join("")}</div>`,
  ).join("");
  ...
  const words = Array.from(root.querySelectorAll<HTMLElement>(".ob-word"));
  const petal = root.querySelector<HTMLElement>(".ob-petal");
  const promptWords = Array.from(root.querySelectorAll<HTMLElement>(".ob-prompt__word"));
  const photos = Array.from(root.querySelectorAll<HTMLElement>(".ob-photo")).map((node, i) => ({
    node,
    depth: PHOTOS[i].depth,
    dir: PHOTOS[i].dir,
  }));
  const enterReduced = root.querySelector<HTMLButtonElement>('[data-ob="enter-reduced"]');
  ...
  function frame(now: number) {
    ...
    const pacedCeiling =
      100 * (runT < 0.5 ? 4 * runT * runT * runT : 1 - Math.pow(-2 * runT + 2, 3) / 2);
    ...
    const cap = modelReady ? 100 : Math.min(indeterminate ? 95 : progressTarget, 99.2);
    ...
    if (yearsOpacity > 0.002) {
      ...
      if (reduced) {
        ...
        for (let j = 0; j < slots.length; j++) {
          ...
          slot.style.filter = ad > 0.06 ? `blur(${(ad * 1.2).toFixed(2)}px)` : "none";
    ...
    const maxScroll = Math.max(1, el.scroller.scrollHeight - el.scroller.clientHeight);
    const vh = H / 100;
    const sc = unlocked ? el.scroller.scrollTop / vh : 0;
    const maxVh = maxScroll / vh;
    const A = 0.36 * maxVh;

    const moved = Math.abs(mx - lastMx) > 0.0004 || Math.abs(my - lastMy) > 0.0004;
    ...
    el.headline.style.transform = `translate3d(0,${stops(sc, [
      [0, 95], [A, 0], [A + 40, -4], [A + 200, -30],
    ]).toFixed(2)}vh,0)`;
    el.headline.style.opacity = String(stops(sc, [[0, 1], [A + 50, 1], [A + 150, 0]]));

    // Word-by-word highlight: each word eases its own ramp, so the wipe reads as
    // one continuous sweep rather than a row of linear steps.
    const hp = clamp((sc - 0.04 * maxVh) / (0.3 * maxVh));
    for (let i = 0; i < words.length; i++) {
      const wi = ease(clamp((hp * 1.6 - i * 0.048) / 0.34));
      const g = Math.round(226 + (26 - 226) * wi);
      words[i].style.color = `rgb(${g},${g},${g})`;
    }
    if (petal) petal.style.opacity = String(ease(clamp((hp * 1.6 - 0.24) / 0.34)));
    ...
    const split = stops(sc, [[0, 0], [A + 10, 0], [A + 230, 88]]);
    for (let i = 0; i < photos.length; i++) {
      ...
      const rise = stops(sc, [[0, 20], [A, 0]]) * (0.5 + f.depth * 0.18);
    ...
    const bg = Math.round(255 - 17 * clamp(stops(sc, [[0, 0], [A + 300, 0], [A + 360, 1]])));
    ...
    for (let i = 0; i < promptWords.length; i++) {
      const wi = clamp((v - i * 0.045) / 0.38);
      promptWords[i].style.opacity = String(wi);
      promptWords[i].style.transform = `translateY(${((1 - wi) * 14).toFixed(2)}px)`;
    }
  ...
  return {
    setProgress({ progress, loaded, total }) {
      ...
      if (progress !== null) progressTarget = Math.max(progressTarget, progress * 100);
...
/** Rewrite the precomposed Vietnamese letters Libertinus Serif renders blank. */
function fixGlyphs(scope: HTMLElement) {
  const walker = document.createTreeWalker(scope, NodeFilter.SHOW_TEXT);
  let node: Node | null;
  while ((node = walker.nextNode())) {
    const value = node.nodeValue;
    if (!value) continue;
    const out = value.replace(GLYPH_RE, (c) => GLYPH_FIX[c]);
